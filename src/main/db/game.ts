...
export function createTache(db: Db, input: { titre: string; description?: string | null; niveauEnergie?: NiveauEnergie; dureeEstimeeMin?: number; categorie?: string | null }): TacheDTO {
  const energie = input.niveauEnergie ?? 'faible'
  const res = db.prepare(`
    INSERT INTO taches (titre, description, niveau_energie, duree_estimee_min, xp_recompense, coins_recompense, categorie)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(
    input.titre,
...
export function updateTache(db: Db, id: number, input: Partial<{ titre: string; description: string | null; niveauEnergie: NiveauEnergie; dureeEstimeeMin: number; categorie: string | null }>): TacheDTO {
  const fields: string[] = []
